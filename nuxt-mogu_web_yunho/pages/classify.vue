...
<script>
import { getBlogSortList, getArticleByBlogSortUid } from "~/assets/scripts/classify";
export default {
  data() {
    return {
      selectBlogUid: "",
  ...
  methods: {
    getBlogList(blogSortUid) {
      ...
      getArticleByBlogSortUid(params).then(response => {
        if (response.data.code == this.$ECode.SUCCESS) {
          this.itemByDate = response.data.data.records;
    ...
    load() {
      ...
      getArticleByBlogSortUid(params).then(response => {
        if (response.data.code == this.$ECode.SUCCESS) {
          this.itemByDate = this.itemByDate.concat(response.data.data.records);
          this.currentPage = response.data.data.current;
          this.pageSize = response.data.data.size;
        }
      });
