<template>
  <article>
    ...
    <div class="infosbox">
      ...
      <!--付款码和点赞 v-if="openAdmiration == '1'"-->
      <PayCode :blogUid="blogUid" :praiseCount.sync="blogData.collectCount"></PayCode>
...
<script>
    ...
    import { getBlogByUid, getSameBlogByBlogUid } from "~/assets/scripts/blogContent";
    ...
    import { addComment, getCommentList } from "~/assets/scripts/comment";
...
import web from '~/assets/scripts/config/webconst'
    ...
    export default {
      ...
      head(){
        console.log("info head run");
        // console.log(this.blogData);
        return this.$seo("博客详情-"+this.blogData.title,this.blogData.title,this.blogData.summary,[{}]);
      ...
      async asyncData({params,query,app,$axios}){
        var that = this;
        // console.log("info asyncData run")
        // console.log(query.blogOid)
        
        var paramsIn = new URLSearchParams();
        if(query.blogUid) {
          paramsIn.append("uid", query.blogUid);
        }
        if(query.blogOid) {
          paramsIn.append("oid", query.blogOid)
        }
        let res = await getBlogByUid(paramsIn);
        // console.log("getBlogByUid ********");
        // console.log(res.data);
        if (res.data.code == app.$ECode.SUCCESS) {
          // retData.blogData = res.data;
          // retData.blogUid = res.data.uid
          // retData.blogOid = res.data.oid
          // retData.commentInfo.blogUid = res.data.uid;
          // document.title = res.data.title
          // console.log(res.data.data.uid);
          return {  
            blogData:res.data.data,
            blogUid:res.data.data.uid,
            blogOid:res.data.data.oid,
            commentInfo:{
              // 评论来源： MESSAGE_BOARD，ABOUT，BLOG_INFO 等 代表来自某些页面的评论
              source: "BLOG_INFO",
              blogUid:res.data.data.uid
        ...
        created() {
            ...
            this.blogUid = this.blogUid?this.blogUid:this.$route.query.blogUid?this.$route.query.blogUid:'';
            this.blogOid = this.blogOid?this.blogOid:this.$route.query.blogOid?this.$route.query.blogOid:'';
        ...
        methods: {
            ...
            getSameBlog() {
              ...
              getSameBlogByBlogUid(blogParams).then(response => {
                if (response.data.code == this.$ECode.SUCCESS) {
                  this.sameBlogData = response.data.data.records;
                }
              });
            },
            // 设置是否开启评论和赞赏
            setCommentAndAdmiration() {
              let webConfigData = this.$store.getters.getWebConfigData;
              // console.log("infowebConfigData");
              // console.log(webConfigData);
              // console.log(webConfigData.createTime);
              if(webConfigData && webConfigData.createTime) {
            ...
            getCommentDataList: function() {
                let params = {};
                // console.log("getCommentDataList******");
