...
<script>
  ...
  import {addComment, deleteComment, getCommentList, reportComment} from "~/assets/scripts/comment";
  ...
  export default {
    ...
    methods: {
      ...
      getDictList: function () {
        if(this.$store.state.userTagDictList.length > 0) {
          this.userTagDictList = this.$store.state.userTagDictList
          return;
        }
      ...
      submitBox(e) {
        ...
        addComment(params).then(response => {
            if (response.code == this.$ECode.SUCCESS) {
              ...
              let comments = this.$store.state.commentList;
