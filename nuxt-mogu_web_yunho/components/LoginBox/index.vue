...
<script>
  import {login, localLogin, localRegister, getWechatOrCodeTicket, getUserLoginStatus} from "~/assets/scripts/user";
  import { Loading } from 'element-ui';
  import {setCookie} from "~/assets/scripts/util/cookieUtils";
          ...
          if(!valid) {
            ...
            localLogin(params).then(response => {
              if (response.data.code == this.$ECode.SUCCESS) {
                // 跳转到首页
                location.replace(this.vueMoguWebUrl + "/#/?token=" + response.data.data)
