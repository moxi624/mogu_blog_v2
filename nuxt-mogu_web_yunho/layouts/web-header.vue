...
<script>
  ...
  import {getWebConfig, getWebNavbar} from "~/assets/scripts/index";
  import {delCookie, getCookie, setCookie} from "~/assets/scripts/util/cookieUtils";
  import {authVerify, editUser, updateUserPwd, replyBlogLink, deleteUserAccessToken, getFeedbackList, addFeedback} from "~/assets/scripts/user";
  import {getCommentListByUser, getPraiseListByUser, getUserReceiveCommentCount, readUserReceiveCommentCount} from "~/assets/scripts/comment";
  import LoginBox from "~/components/LoginBox";
  import {getListByDictTypeList} from "~/assets/scripts/sysDictData"
  // vuex中有mapState方法，相当于我们能够使用它的getset方法
  import {mapMutations} from 'vuex';
  import {timeAgo} from "~/assets/scripts/util/webUtils";
  ...
  export default {
    ...
    watch: {
      ...
      // 判断登录状态位是否改变（用于控制弹框）
      '$store.state.loginMessage': function (newFlag, oldFlag) {
          this.showLogin = true
      }
    },
    created() {
    ...
    methods: {
      ...
      ...mapMutations('user',['setUserInfo', 'setLoginState']),
      // 搜索
      search: function () {
      ...
      getWebNavbarList() {
        ...
        getWebNavbar(params).then(response => {
          if(response.data.code == this.$ECode.SUCCESS) {
            ...
            let webNavbarList = response.data.data
      ...
      getFeedback: function() {
        ...
        getFeedbackList(params).then(response => {
          if(response.data.code == this.$ECode.SUCCESS) {
            this.feedbackList = response.data.data.records;
          }
        })
      },
      ...
      getPraiseList: function() {
        ...
        getPraiseListByUser(params).then(response => {
          if(response.data.code == this.$ECode.SUCCESS) {
            this.praiseList = response.data.data.records;
          }
        })
      },
      ...
      setUserReceiveCommentCount: function () {
        getUserReceiveCommentCount().then(response => {
          ...
          if (response.data.code == this.$ECode.SUCCESS) {
            this.userReceiveCommentCount = response.data.data
          }
        });
      },
      getKeyword: function() {
      ...
      userLogout: function () {
        ...
        delCookie("token");
        let url = this.window.parent.location.href;
